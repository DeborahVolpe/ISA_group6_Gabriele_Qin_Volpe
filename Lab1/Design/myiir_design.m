...
xlabel('\textit{Normalized frequency $(\frac{f}/{\Bigg(f_{sample}/2\Bigg)}$} ','Interpreter','latex','FontSize', 14);
ylabel('\textit{dB}','Interpreter','latex','FontSize', 14);
title ('\textbf{IIR Filter transfer function}','Interpreter','latex','FontSize', 14);
xlim([0 1])
ylim([-70 0])
legend('\textit{designed filter}','\textit{quantized filter}','Interpreter','latex','Location','southwest', 'FontSize', 12)
grid on

print -depsc IIR_design
